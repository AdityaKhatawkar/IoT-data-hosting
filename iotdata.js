...
const MONGO_URI =
  "mongodb+srv://Aditya_Khatawkar:1234@kvb-iot.3xqiiv3.mongodb.net/mydatabase?retryWrites=true&w=majority";
...
app.post("/iotdata", async (req, res) => {
  try {
    ...
    // Keep only the 3 latest readings, delete older ones
    const readings = await DeviceData.find()
      .sort({ timestamp: -1 })
      .skip(3); // skip the 3 latest
...
// Get latest data (for testing in dashboard)
